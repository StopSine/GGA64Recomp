...
cp GGA64Recompiled AppDir/usr/bin/
cp -r assets/ AppDir/usr/bin/
cp recompcontrollerdb.txt AppDir/usr/bin/
cp icons/512.png AppDir/GGA64Recompiled.png
cp .github/linux/GGA64Recompiled.desktop AppDir/

"./linuxdeploy-$LINUX_DEPLOY_ARCH.AppImage" --appimage-extract
mv squashfs-root/ deploy
./deploy/AppRun --appdir=AppDir/ -d AppDir/GGA64Recompiled.desktop -i AppDir/GGA64Recompiled.png -e AppDir/usr/bin/GGA64Recompiled --plugin gtk
...
echo '    APP_FOLDER_PATH=$APP_FOLDER_PATH ./GGA64Recompiled' >> AppDir/AppRun
echo 'else' >> AppDir/AppRun
echo '    cd "$this_dir"/usr/bin/' >> AppDir/AppRun
echo '    ./GGA64Recompiled' >> AppDir/AppRun
